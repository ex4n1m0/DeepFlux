...
class DownloadEngine:
    ...
    def add_job(
        ...
        if not save_path:
            ...
            with self._lock:
                save_path = self._unique_save_path(self._default_folder, filename)
        else:
        ...
        threading.Thread(
            target=self._probe_and_setup, args=(job,), daemon=True, name=f"probe-{job.id}"
        ).start()
    ...
    def add_stream_job(
        ...
        # Parse the manifest.
        info = parse_manifest(url, headers=headers, cookies=cookies, referrer=referrer)
        if info.error:
    ...
    def _start_stream_job(self, job: DownloadJob) -> None:
        ...
        def _stream_worker():
            try:
                ...
                info = parse_manifest(job.url, headers=job.headers, cookies=job.cookies, referrer=job.referrer)
                if info.error:
        ...
        thread = threading.Thread(target=_stream_worker, daemon=True, name=f"stream-{job.id}")
        thread.start()
        return job
    ...
    def add_youtube_job(
        ...
        if not save_path:
            ...
            with self._lock:
                save_path = self._unique_save_path(self._default_folder, base)
        else:
            os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
        self._register_folder(save_path)
    ...
    def _start_youtube_worker(self, job: DownloadJob) -> None:
        ...
        thread = threading.Thread(target=_yt_dlp_worker, daemon=True, name=f"youtube-{job.id}")
        thread.start()
    ...
    def _try_start_jobs(self) -> None:
        ...
        with self._lock:
            active = sum(1 for j in self._jobs.values() if j.status == JobStatus.DOWNLOADING)
            for job in self._jobs.values():
