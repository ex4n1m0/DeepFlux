...
"""PyInstaller spec for DeepFlux on macOS (.app bundle, onedir inside).

Built by CI on a macOS runner (see .github/workflows/macos-build.yml) —
PyInstaller cannot cross-compile, so this spec only ever runs on a Mac.

Differences from app.spec (Windows):
  * No Play tab: gui.iptv_tab / gui.iptv_settings_dialog are conditionally
    imported behind _PLAY_TAB_SUPPORTED (False on darwin), so they never
    enter the bundle. The iptv.* pure-python modules may still be pulled in
    by agent/tools.py's lazy imports — harmless dead code.
  * No libmpv / libVLC DLLs, no bundled FFmpeg (agent installs via Homebrew
    and points download.ffmpeg_path at it — see the SYSTEM_PROMPT note), no
    MilkDrop visualizer files (player-only).
  * BUNDLE() step produces DeepFlux.app with an .icns icon and the
    microphone usage description voice input needs (missing it makes macOS
    kill the process on first mic access).
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        # mac build ships without shared keys (users enter their own).
    ...
    runtime_hooks=[],
    excludes=[
        # Player backends never load on macOS (no Play tab) — and python-mpv
...
app = BUNDLE(
    ...
    bundle_identifier="space.deepflux.DeepFlux",
    version=app_version,
    info_plist={
        # Voice input records from the default microphone.
        "NSMicrophoneUsageDescription":
            "DeepFlux uses the microphone for offline voice typing in the Agent box.",
        "NSHighResolutionCapable": True,
        # Chromium's own sandbox helpers inside the Qt frameworks.
        "CFBundleInfoDictionaryVersion": "6.0",
        "CFBundleShortVersionString": app_version,
        "CFBundleLongVersionString": app_version,
    },
)
